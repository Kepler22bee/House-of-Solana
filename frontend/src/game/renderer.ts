...
import { Player, drawPlayer, MapData, getFacingTile, charSheets, CHAR_COUNT, FRAME_W, FRAME_H, FRAMES_PER_DIR, DIR_ROW } from "./player";
import { isNpcBored } from "./npc-ai";
...
export function renderGame(
  ...
  canvasW: number,
  canvasH: number,
  dialogueState: { active: boolean; npc: NPC | null; line: number; lines?: string[] } | null,
...
function drawTablePointers(ctx: CanvasRenderingContext2D) {
  ...
  const tables = [
    ...
    { label: "🃏 BLACKJACK", color: "#4caf50", x: 25, y: 22 },
...
/* ── NPC sprite rendering (Memao Fantasy characters) ── */

// Assign each NPC a unique character index (1–10) from their name
const npcCharIdx: Record<string, number> = {};
function getNpcSheetIdx(name: string): number {
  if (!(name in npcCharIdx)) {
    let h = 0;
    for (let i = 0; i < name.length; i++) h = (h * 31 + name.charCodeAt(i)) & 0xffff;
    // Skip index 0 (player uses char1), NPCs use chars 2-10
    npcCharIdx[name] = 1 + (h % (CHAR_COUNT - 1));
  }
  return npcCharIdx[name];
}
...
function drawNPC(ctx: CanvasRenderingContext2D, npc: NPC, player: Player) {
  ...
  const idx = getNpcSheetIdx(npc.name);
  const sheet = charSheets[idx];
  ...
  const row = DIR_ROW[facing];
  const srcX = 1 * FRAME_W;
  const srcY = row * FRAME_H;

  ctx.drawImage(sheet, srcX, srcY, FRAME_W, FRAME_H, px, py - 10 + bob, drawSize, drawSize + 10);
  ...
  if (dist < 80) {
    ...
    // E to interact hint (hide if NPC is bored)
    if (!isNpcBored(npc.name)) {
...
function drawUI(
  ...
  if (activeDialogue && activeDialogue.npc) {
    ...
    drawDialogueBox(ctx, canvasW, canvasH, activeDialogue.npc.name, dlgText, activeDialogue.npc.name);
...
function drawDialogueBox(
  ...
  const avatarW = 100;
  const gap = 10;
  const boxH = nameTagH + panelH + pad * 2 + 4; // name + gap + panels + padding
  ...
  // Name tag — takes whatever width it needs
  ctx.font = "bold 18px 'Courier New', monospace";
  const nameW = ctx.measureText(name).width + 28;
  const nameTagW = Math.max(nameW, 120);
  const nameTagX = boxX + pad;
  const nameTagY = boxY + pad;
  ctx.fillStyle = "#b8a880";
  ctx.beginPath();
  ctx.roundRect(nameTagX, nameTagY, nameTagW, nameTagH, 4);
  ctx.fill();
  ...
  ctx.fillStyle = "#3a3020";
  ctx.textAlign = "left";
  ctx.fillText(name, nameTagX + 12, nameTagY + 22);

  // Panels row — avatar and text at same height & same height
  const panelY = nameTagY + nameTagH + 4;

  // Avatar panel (left) — only for NPC dialogues
  if (npcName) {
    const apX = boxX + pad;
    ctx.fillStyle = "#b8a880";
    ctx.beginPath();
    ctx.roundRect(apX, panelY, avatarW, panelH, 6);
    ctx.fill();
    ctx.strokeStyle = "#9a9080";
    ctx.lineWidth = 2;
    ctx.stroke();

    const aIdx = getNpcSheetIdx(npcName);
    const aSheet = charSheets[aIdx];
    if (aSheet && aSheet.complete) {
      const aSrcX = 1 * FRAME_W;
      const aSrcY = DIR_ROW["down"] * FRAME_H;
      const cropX = aSrcX + 6;
      const cropY = aSrcY + 5;
      const cropW = FRAME_W - 12;
      const cropH = 50;
      ctx.drawImage(aSheet, cropX, cropY, cropW, cropH,
        apX + 6, panelY + 4, avatarW - 12, panelH - 8);
    }
  }

  // Text panel — same height as avatar
  const tpX = npcName ? boxX + pad + avatarW + gap : boxX + pad;
  const tpW = npcName ? boxW - pad * 2 - avatarW - gap : boxW - pad * 2;
  ctx.fillStyle = "#ede6d0";
  ctx.beginPath();
  ctx.roundRect(tpX, panelY, tpW, panelH, 6);
  ctx.fill();
  ctx.strokeStyle = "#a89878";
  ctx.lineWidth = 2;
  ctx.stroke();
  ...
  const textStartY = panelY + (panelH - totalTextH) / 2 + 10;
  for (let i = 0; i < lines.length; i++) {
    ctx.fillText(lines[i], tpX + 12, textStartY + i * lineH);
  }
